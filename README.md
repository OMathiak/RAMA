...
We included a persistency based preprocessor that can reduce the effective size of the input graph, while retaining the optimal solution. The gains from the prepocessor are heavily dependent on the structure of the graph. It works best for only loosly connected graphs. However, due to its light weight, in the worst case scenario it only impacts the performance marginally.
...
- `--preprocessor_each_step <THRESHOLD>`: The preprocessor will stop if less than THRESHOLD of the graphs edges are found to be persistent
